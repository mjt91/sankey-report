...
class SankeyDiagramGenerator:
    ...
    def prepare_sankey_data(self):
        ...
        # Get unique nodes
        all_nodes = list(
            set(self.data["source"].tolist() + self.data["target"].tolist())
        )
    ...
    def create_sankey_diagram(self, title="Sankey Diagram", width=1200, height=800):
        ...
        # Create node colors
        node_colors = [
            self.colors[i % len(self.colors)] for i in range(len(sankey_data["nodes"]))
        ]
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate beautiful Sankey diagrams from CSV data"
    )
    parser.add_argument(
        "--csv", type=str, help="Path to CSV file (columns: source,target,value)"
    )
    parser.add_argument(
        "--output", type=str, default="sankey_diagram.html", help="Output file path"
    )
    parser.add_argument(
        "--title", type=str, default="Sankey Diagram", help="Chart title"
    )
    ...
    if not args.csv:
        print(
            "Error: Please provide a CSV file with --csv or use --sample to create sample data"
        )
        parser.print_help()
        sys.exit(1)
